...
func generateRoundMaterial(seed []byte, out *Construction, hidden func(int, int) table.DoubleToWord) {
...
func generateBarriers(seed []byte, out *Construction, inputMask, outputMask, sr *matrix.Matrix) {
	// Generate the ShiftRows and re-encoding matrices.
	out.ShiftRows[0] = MaskSwap(seed, 16, 0).Compose(*sr).Compose(*inputMask)

	for round := 1; round < 10; round++ {
		out.ShiftRows[round] = MaskSwap(seed, 16, round).Compose(*sr).Compose(MaskSwap(seed, 32, round-1))
	}
...
func GenerateEncryptionKeys(key, seed []byte, opts common.KeyGenerationOpts) (out Construction, inputMask, outputMask matrix.Matrix) {
	...
	generateRoundMaterial(seed, &out, hidden)
	generateBarriers(seed, &out, &inputMask, &outputMask, &ShiftRows)
...
func GenerateDecryptionKeys(key, seed []byte, opts common.KeyGenerationOpts) (out Construction, inputMask, outputMask matrix.Matrix) {
	...
	common.GenerateMasks(seed, opts, &inputMask, &outputMask)
	generateRoundMaterial(seed, &out, hidden)
	generateBarriers(seed, &out, &inputMask, &outputMask, &UnShiftRows)
